...
x_training_values = train_data[['x1', 'x2']]  # here i will get the first two columns of the data which are the features
y_training_values = train_data['y']  # here i will get the last column of the data which is the actual value of y
x_validation_values = validation_data[['x1', 'x2']]  # same here
y_validation_values = validation_data['y']  # same here
...
for degree in range(1, 11):
    ...
    X_train_polynomial = feature_degree_function.fit_transform(x_training_values)  # here i will substitute the train data in the function to get the target feature

    X_val_polynomial = feature_degree_function.transform(x_validation_values)  # same here but with validation data

    model = LinearRegression(fit_intercept=True)  # makeing the model of linear regression to substitute the data in it to get the target feature.

    model.fit(X_train_polynomial, y_training_values)  # here i will substitute the data in the model to train it.
    ...
    validation_error = mean_squared_error(y_validation_values,y_val_pred)  # now i will compute the mean squared error of the validation data.
    ...
    x1_range = np.linspace(x_training_values['x1'].min(), x_training_values['x1'].max(), 100)
    x2_range = np.linspace(x_training_values['x2'].min(), x_training_values['x2'].max(), 100)
    ...
    ax.scatter(x_training_values['x1'], x_training_values['x2'], y_training_values, color='r')
    ax.set_xlabel('X1')
    ax.set_ylabel('X2')
    ax.set_zlabel('Y')
...
# Prepare data
x_training_values = train_data[['x1', 'x2']]  # same as before
y_training_values = train_data['y']
x_validation_values = validation_data[['x1', 'x2']]
y_validation_values = validation_data['y']
...
X_train_poly = feature_degree_function.fit_transform(x_training_values)
X_val_poly = feature_degree_function.transform(x_validation_values)
...
for alpha in alphas:
    ...
    model.fit(X_train_poly, y_training_values)  # here i will substitute the data in the model to train it.

    y_val_pred = model.predict(X_val_poly)  # getting the predicted values of the validation data.

    val_error = mean_squared_error(y_validation_values,
                                   y_val_pred)  # computing the mean squared error of the validation data.
...
# Prepare the data
x_training_values = train_data.drop('class',
                                    axis=1)  # here i sliced the data to get just the first two columns of features
y_training_values = train_data['class']  # here i saved the last column of the data which is the class
X_testing_values = test_data.drop('class', axis=1)  # same here
y_testing_values = test_data['class']  # same here
...
model.fit(x_training_values,
          y_training_values)  # here I substituted the data to the model to train it  the two features , and classes .

# here i will plot the decision boundary of the model

x_min = x_training_values[
            'x1'].min() - 1  # here i will get the minimum value of x1 and subtract 1 from it to ensure that the plot covers all the data
x_max = x_training_values[
            'x1'].max() + 1  # here i will get the maximum value of x1 and add 1 to it to ensure that the plot covers all the data
y_min = x_training_values['x2'].min() - 1  # same
y_max = x_training_values['x2'].max() + 1  # same

xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.02),
                     np.arange(y_min, y_max, 0.02))  # here i will create a mesh grid to plot the data
...
plt.scatter(x_training_values['x1'], x_training_values['x2'], c=y_training_values, edgecolors='k', cmap=plt.cm.Paired)
...
# finding  accuracies
train_accuracy = accuracy_score(y_training_values, model.predict(
    x_training_values))  # here i will compute the accuracy of the model on the train data to print it to cmpare it with the test data
test_accuracy = accuracy_score(y_testing_values, model.predict(
    X_testing_values))  # here i will compute the accuracy of the model on the test data.
...
features_degree = PolynomialFeatures(degree=2,
                                     include_bias=False)  # here i make the funcions  features quadratic x1^2 , x2^2 , x1x2 , x1 , x2 , 1

X_train_features_degree = features_degree.fit_transform(
    x_training_values)  # i substituted the train data in function to get the quadratic features
X_test_features_degree = features_degree.transform(X_testing_values)  # same here but with test data

# make the model of logistic regression

model_quad = LogisticRegression()  # same as before
model_quad.fit(X_train_features_degree, y_training_values)  # same as before but with the quadratic features.
...
# finding accuracies
train_accuracy_quad = accuracy_score(y_training_values, model_quad.predict(X_train_features_degree))
test_accuracy_quad = accuracy_score(y_testing_values, model_quad.predict(X_test_features_degree))
